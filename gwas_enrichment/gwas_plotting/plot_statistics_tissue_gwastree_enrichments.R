...
if(use.cancersecondary){
    ...
    # Merge fix regdf:
    regdf = regdf[,c('node','uid','log10p','isleaf','type')]
    regdf = merge(regdf, nodetissue)
    regdf$category = factor(regdf$category, levels=levels(odf$category))
...
boxplot(log10p ~ GROUP, data=regdf, horiz=F,
...
# pdf(paste0(imgpref, 'ngwas_summajority_group.pdf'), width=8,height=4)
par(mar=c(5,3,1,.5))
par(yaxs='i')
par(xaxs='i')
# minor = 50
...
plot(cs2 + 0.5, pct[pord], xaxt='n', yaxt='n',
     ...
     ylim=c(0,50),xlim=c(0.1, max(cs2) + 1), xpd=TRUE)
lines(cs2 + 0.5, pct[pord], col=tsp.col('indianred'))
text(cs2 + 0.5, 45, paste0(pct[pord], '%'), cex=.5, srt=90,adj=0, col='indianred')
atpar=seq(0,50, 10)
...
# TODO: Need to take ratio correctly against all UID
...
# ----------------------------------------------
# TODO: V2 should somehow correct for adult vs. embryonic tissue.
# - potentially swap keeping the tissue + embryonic/adult margin?
    ...
    for (t in 1:ncol(nodetismat)){
        ...
        for (i in 1:NPERM){
            ...
            if (metric == 'cosine'){
                ...
                # TODO: Find better way for not undercounting here.
...
dt$cex[dt$pvalue < 0.05] = 2
dt$cex[dt$pvalue < 0.01] = 3
dt$cex[dt$pvalue < 0.001] = 4
dt$border = 'black'
...
# TODO: Make the legend with the base R. looks better.
...
legend('bottom', legend=c('NS','p < 0.05','p < 0.01','p < 0.001'), title='p-value',
       col='black', fill='white', pch=22, pt.cex=c(1,2,3,4), bty='n', border=NA, cex=1, horiz=T, inset=c(0,-.04))
...
# TODO: set size matters for 'urinary' set
...
pnodemat = all.regmat[kw,] > cutoff
pnodemat = pnodemat[guid,]
pnodetismat = pnodemat %*% ntmat
pnodetismat = pnodetismat[apply(pnodetismat,1, sum) > 0,]
pnodetismat = sweep(pnodetismat, 1, apply(pnodetismat, 1, sum), '/')
range(pnodetismat)
