...
# --------------------------------
# Process/plot a single (all - 1% FDR) matrix:
...
dim(gwdf[gwdf$uid %in% keptgw$uid,]) # 71,379 
...
# ----------------------------
h = 17 * ncol(epimat) / 400
# png(paste0(imgpref, "epigenomes_gwas_flathg_top", SHOWGWAS, sub(".Rda", "",suffix), "_groupord_diag_flip.png"),res=450,units='in',width=12,height=h)
pdf(paste0(imgpref, "epigenomes_gwas_flathg_top", SHOWGWAS, sub(".Rda", "",suffix), "_groupord_diag_flip.pdf"), width=12, height=h)
layout(matrix(c(1,2),2,1), heights=c(1.5,10), TRUE)
par(yaxs="i")
par(xaxs="i")
par(xpd=FALSE)
par(mar=c(0, 15, 5, 0.25))
...
diagord2 = diagord
diagord2[sapply(diagord2, nchar) > 120] = ''
...
png(paste0(imgpref, "epigenomes_gwas_flathg_matrixonly_resource.png"), res=600,units='in',width=7.75,height=4)
par(mar=rep(0,4))
image(epimat[rnorder, rev(diagord)], axes=FALSE,col=colred, zlim=c(zmin, zmax), useRaster=TRUE)
dev.off()

# png(paste0(imgpref, "epigenomes_gwas_flathg_top", SHOWGWAS, sub(".Rda", "",suffix), "_groupord_diag_flip_repr.png"),res=450,units='in',width=7.75,height=4)
pdf(paste0(imgpref, "epigenomes_gwas_flathg_top", SHOWGWAS, sub(".Rda", "",suffix), "_groupord_diag_flip_repr.pdf"), width=7.75, height=4)
sp = 0.15
rsp = 1.25
lsp = 13.5
layout(matrix(c(1,2),2,1), heights=c(2.25,7), TRUE)
par(yaxs="i")
par(xaxs="i")
par(xpd=FALSE)
...
wcls = c()
# TODO: Down-weight very long gwas
# TODO: Weight by significance of enrichment.
for (i in 1:nrow(rdf)){
    wind = which(vcut == rdf$cls[i])
    subrn = diagord2[wind]
    # if (length(subrn) > 5){ filt=TRUE } else { filt=FALSE }
...
# TODO: make both vertical and horizontal:
aggregate(uid ~ type, typedf, length)
...
satcols = colvals[['project']][as.character(satdf$Project)]
satgroupcols = colvals[['group']][as.character(satdf$GROUP)]
...
# TODO: Working on- rarefaction of maximal enrichments -- could add some of assoc. traits.
rdf = passdf
