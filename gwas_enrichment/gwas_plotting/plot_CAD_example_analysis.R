...
# TODO: Plot trees of signif overlap between examp + other traits
# -- may need to move up consensus tree
# TODO: Get signif. modules that overlap  (send to ypp)
# TODO: Module enrichments, enh annotations, GO terms.
...
# TODO: rerun -only ran for top 20 nodes.
cad.gofile = paste0(regpref, examp,'_go_nodediff_enh.Rda')
if (!file.exists(cad.gofile)){
    # Repopulate, given changed FDR thresh:
    # load(cad.gofile)
    # kept.nodes = cad.nodes[nord][cad.nodes[nord] %in% cad.godf$node]
    # missing.nodes = cad.nodes[nord][!(cad.nodes[nord] %in% cad.godf$node)]
    # for (ni in missing.nodes){
    cad.godf = c()
    for (ni in cad.nodes){
...
cad.nodes[nord][!(cad.nodes[nord] %in% sub.godf$node)]

go.keep = c()
for(ni in cad.nodes[nord]){ go.keep = c(go.keep, head(sub.godf$Description[sub.godf$node == ni], 2)) }
...
shmat = all.regmat[keptgwas, cad.nodes[nord]]
...
if (plot.tree){
    ...
    circleplot(dend=dd$dend, lab=lab, udf=dd$udf,
               ...
               fractional=FALSE, hit.track=dd$hitdf, leaf.pval=all.regmat[,suid])
    upViewport()
    circos.clear()
...
# plot(1, type='n', axes=F, ylab='', xlab='')
box(lwd=.5)
# - TODO: GO enrichments for clustered matrix
