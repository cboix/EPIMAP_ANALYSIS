...
# --------------------------------------------------------
# Run as: 
# qsub -cwd -P compbio_lab -t 1-17938 -l h_vmem=4G -l h_rt=00:10:00 -N conv_wg_bdg -j y -b y -V -r y -o $DBDIR/out/ChromImpute "$BINDIR/make_bw_for_release.sh"
# qsub -cwd -P compbio_lab -t 1-3 -l h_vmem=5G -l h_rt=00:10:00 -N conv_wg_bdg -j y -b y -V -r y -o $DBDIR/out/ChromImpute "$BINDIR/make_bw_for_release.sh"
# Directories/Config:
...
if [[ "$( echo ${FILE} | grep "impute" - -c )" == "1" ]]; then
    ...
    FINALBW=${BWDIR}/imputed_${SAMPLE}_${MARK}.bigWig
else
    bdgdir=${CONVERTED_DIR}
    BWDIR=${PUBLIC_BWDIR}/observed/${MARK}
    BDGDIR=${PUBLIC_BDGDIR}/observed/${MARK}
    FINALBW=${BWDIR}/observed_${SAMPLE}_${MARK}.bigWig
fi
mkdir -p $BWDIR
