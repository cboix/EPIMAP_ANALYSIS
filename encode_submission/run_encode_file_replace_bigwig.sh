...
# ------------------------------------------------
# Run as: 
# qsub -cwd -t 2-223 -tc 500 -l h_vmem=3G -l h_rt=00:45:00 -N dcc_expt_replace_bw -j y -b y -V -r y -o $DBDIR/out/ChromImpute "$BINDIR/encode_submission/run_encode_file_replace_bigwig.sh"
# Directories/Config:
...
if [[ "$nrun" == "1" ]]; then
    # Get dataset type + URL:
    datatype=$( echo "$FILENAME" | awk '{if ($1 ~ /impute/){a = "imputed"} else {a = "observed"}; print a}' ) 
