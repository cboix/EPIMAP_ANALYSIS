...
regdf$in.20k = 1 * (regdf$uid %in% keptgw$uid)
nldf$in.20k = 1 * (nldf$uid %in% keptgw$uid)
regdf = regdf[regdf$nsnp > 0,]
regdf = merge(regdf, data.frame(suffix=suffixes, 
                                fdr.level=fdrs))
regdf$fdr.level = factor(regdf$fdr.level, sort(fdrs))


gwdf$in.20k = 1 * (gwdf$uid %in% keptgw$uid)
sum(gwdf$in.20k) # 66801 SNPs
...
nldf = merge(nldf , data.frame(suffix=suffixes, 
                               fdr.level=fdrs))
nldf$fdr.level = factor(nldf$fdr.level, sort(fdrs))
