...
# All matched tables:
if [[ ! -s ${MATCH_TRACKS_TAB} ]]; then
    awk -vOFS="\t" '{print $1"_"$2,$1,$2,$3}' ${ALL_TRACKS_TAB} | sort > $CIDIR/tmp.tracks.tsv
    cut -f1 $CIDIR/tmp.tracks.tsv | uniq -c | awk '$1 == 2{print $2}' | join - ${CIDIR}/tmp.tracks.tsv | awk -vOFS="\t" '{if ($4 ~ /FINAL/){$2 = $2"_obs"} else {$2 = $2"_imp"}; print $2,$3,$4}' > ${MATCH_TRACKS_TAB} 
fi
