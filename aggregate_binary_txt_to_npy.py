...
# 3. 200bp DNase-seq (Binary - keep internal)
...
class convert_binary(object):
    def __init__(self, infofile, dir, out, chromhmm=False, states=0,
        ...
        self.dmlpref = os.environ['DML_DIR'] + \
    ...
    def process_index_line(self, line):
        line = line.split("\n")[0].split("\t")
    ...
    # TODO: Process leaving ALL STATES in the matrix (833 x 3.6M is small)
    ...
    def preprocess(self, prefix):
        self.setup()
        idnum = np.where(np.array(self.ids) == prefix)[0][0]
        currid = self.ids[idnum]
        maindir = self.dirs[idnum]
        print("[STATUS] Preprocessing row " + str(idnum) + " of samplesheet.")
        for chrom in self.chrlist:
            print
    ...
    def merge_bins_average(self, X, chrom):
        ...
        # Sparsify by cutting out values less than 2:
        out[out < 2.0] = 0
        csarr = csr_matrix(out)
        return([csarr, chunks])
