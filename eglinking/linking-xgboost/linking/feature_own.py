...
class feature:
    ...
    def gen_length_feature(min_length, max_length, exponent=-1):
        ...
        # return lambda p, n: ((np.abs(p['offset']))**(-1),
        #                      np.abs(np.random.uniform(-min_length,max_length))**(-1))
        # return lambda p, n: (np.abs((p['offset'] - min_length) / (max_length - min_length))**(-1),
                             # np.abs((p['offset'] - min_length) / (max_length - min_length))**(-1))

    def featurize(self, pos_slice, neg_slice, extra_features=[]):
    ...
    # Featurize for logistic:
    def featurize_orig(self, pos_slice, neg_slice, extra_features=None):
        """Collect indices which match those from the positive selection, in the same number.
           This confirms that the same DHS indices are sent to both positive and negative
           in the same number and order."""
        neg_idx = []
