...
system(cmd)
imgpref = paste0(imgdir, 'epi_motif_updated_') # don't overwrite old plots:
...
# ---------------------------------
motif.resfile = 'motifs/collated.bkgdhs.epi.enrich.tsv.gz'
motif.rdafile = 'motifs/collated.bkgdhs.epi.enrich.processed.Rda'
motif.fulltsv = 'motifs/collated.bkgdhs.epi.enrich.processed.full.tsv.gz'
motif.redtsv = 'motifs/collated.bkgdhs.epi.enrich.processed.reduced.tsv.gz'
cutoff = .5
pcutoff = -log10(0.005)
if (!file.exists(motif.rdafile)){
    ...
    # Names:
    namesfile = paste0('cls_merge2_wH3K27ac100_raw', '_names.tsv')
    ...
    rownames(full.mmat) = epinames[rownames(full.mmat)]
...
#     plt.gmat = plt.gmat[apply(abs(plt.gmat),1, max) >= 4,, drop=F]
#     # Motifs:
...
clsgrp = as.character(meta[clsnam, 'GROUP'])
...
irn = rownames(imat)
rownames(imat) = make.unique(meta[rownames(imat), 'infoline'])
names(irn) = rownames(imat)
dim(imat)
...
V(net2)$color[lind] = clscols[irn[lbnames[lind]]]
if (plot.lbl) {
    V(net2)$label.cex= (.4 - .2 * lind)
} else {
    V(net2)$label[lind] = ""
...
l <- layout_with_dh(net2)

png('~/test.png', res=450, units='in', width=7, height=7)
par(mar=rep(0.1, 4))
plot(net2, layout=l, vertex.label.color="black", vertex.size=(2 - 1 * V(net2)$type)*1) 
dev.off()
...
V(net3)$label = ""
E(net3)$width <- 0
# l <- layout_with_fr(net3, grid='nogrid') # Usually best

png('~/test_nonet.png', res=450, units='in', width=7, height=7)
par(mar=rep(0.1, 4))
plot(net3, layout=l, vertex.label.color="black", vertex.size=(2 - 2 * V(net2)$type)*1) 
dev.off()



save(mmat, full.mmat, clscols, file='motifs/motif_epi_network_data.Rda')
