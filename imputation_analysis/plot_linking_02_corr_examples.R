#!/usr/bin/R
# ------------------------------------------------
# Plot the naive correlation based links for genes
# ------------------------------------------------
...
for (k in 1:length(center.genes)){
    ...
    plot.height=2 / 1.2
    png(paste0(traitdir, 'gene_link_vis_', gene, '_w', window, genesuf, '.png'), units='in', res=450, width=plot.width, height=plot.height)
    # arc.sect=3
    arc.sect=2.5
    if (top.arc){
