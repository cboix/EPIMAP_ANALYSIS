...
col_fun = function(x, pal=colryb){
    palette = rev(pal)
    bin <- cut(x, seq(0,1, length.out=length(palette)), include.lowest=T) 
    palette[bin] }
plegend = Legend(at = seq(0, 1, .5), 
                    labels_gp = gpar(fontsize=5),
                    title_gp = gpar(fontsize=5, fontface='bold'),
                    col_fun=col_fun, title_position = "topleft", 
                    title='GWCorr', direction = 'vertical')

pdf(paste0(imgpref,'heatmap_legend.pdf'), width=7.75, height=4.25)
...
# TODO: PLOT COLORS ON DENDROGRAM TOO
# TODO: ADD FETAL/ADULT DISTINCTION
# ------------------------
# Reorder matrix:
dt <- as.dist(full)
ht <- hclust(dt, method='ward.D')
...
# TODO: UPDATE FOR ALL MARKS, sans DNase-seq.
small=TRUE
