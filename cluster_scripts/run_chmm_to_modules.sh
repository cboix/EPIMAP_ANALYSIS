...
# 2. MAKEMAT
# TODO: Make MAKEMAT for H3K27ac + DNase-seq
...
export SPECIFIC="0"
export EXTSIZE="100"
export PREF=""  # TODO Not coded.
# export SMERGE="0"
# export MWITH="0"
export SMERGE="2"
export MWITH="1"
export RAWSIGNAL="0"
export DENSEMATRIX="0"
export NOCLS="0"
export NONOVL="0"
export SEEDED="0"
export MINIMAL="0"
# TODO write in merge H3K27ac and annotation
...
if [ $# -eq 0 ]; then
    ...
    -o     Use non-overlapping DHS index (OPTIONAL - 0/1) [default: $NONOVL]
    -d     Run seeded (for CLUSTER) command [default: $SEEDED]
    -l     Plot/write minimal (for CLUSTER) [default: $MINIMAL]
    -z     Aggregate a dense matrix (OPTIONAL 0/1) [default: $DENSEMATRIX]
    ------------ MODEL INFORMATION ------------
    -n     Number of states model (15, 18, 25) (OPTIONAL) [default: $NUMSTATES]
    -q     Use matched cutoffs (OPTIONAL - 0/1) [default: $QCUT]
    -m     Use mixed obs/imputed data (OPTIONAL - 0/1/2 - use only matched data) [default: $MIXOBS]"
    exit 1
fi

while getopts t:s:p:e:k:g:r:x:w:a:o:d:l:z:n:q:m: o
do      case "$o" in
    ...
    o)      export NONOVL="$OPTARG";;
    d)      export SEEDED="$OPTARG";;
    l)      export MINIMAL="$OPTARG";;
    z)      export DENSEMATRIX="$OPTARG";;
...
# Only operate on dense matrix if raw signal:
if [[ "$RAWSIGNAL" == "0" ]]; then
    DENSEMATRIX=0
fi

if [[ "$DENSEMATRIX" == 1 ]]; then
    JOBSUF="${JOBSUF}dense_"
fi
...
if [[ "$MIXOBS" == "1" ]]; then
    ...
    export JOBSUF="${JOBSUF}on_mixed_impobs"
elif [[ "$MIXOBS" == "0" ]]; then
    export INFOFILE=$IMPOBS_TAB
    export JOBSUF="${JOBSUF}on_imputed"
elif [[ "$MIXOBS" == "2" ]]; then
    echo "[STATUS] Setting up all imputed vs. observed comparisons"
    export INFOFILE=${MATCH_TRACKS_TAB}
    export JOBSUF="${JOBSUF}on_matched"
    # NOTE: Only for aggregate + compare DHS regions for validation analysis.
else
    echo "[WARNING] Unrecognized option for MIXOBS (-m)"
fi
...
# Make the argument list (for processing/aggregating):
...
# Add option of using non-overlapping DHS list:
if [[ "$NONOVL" == "1" ]]; then
    MAINARGS="$MAINARGS --nonovl"
fi

if [[ "$DENSEMATRIX" == "1" ]]; then
    MAINARGS="$MAINARGS --nosparsify"
fi
...
# Also add seed if necessary, where seed is task
if [[ "$SEEDED" == "1" ]]; then
    export CPREFIX=${CPREFIX}_seed${TASK}
fi
