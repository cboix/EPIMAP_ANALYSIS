...
# =======================================================
# 5b. Generate the coeff of var track for a mark + sample
# NOTE: Parallelized over sample/marks
#       Can parallelize over chromosomes with -c $CHROM
# =======================================================
IFS=$'\t'
...
do
    ...
    # 5 hrs should be enough - unless chr1 takes very long. 
    # Matched to predictors array job + can be in parallel
    qsub -cwd -t 1-$FNUM -P compbio_lab -l h_vmem=20G -l h_rt=7:30:00 -N coeffv_${UQID} -j y -b y -V -r y -hold_jid_ad pred_${UQID} -o $DBDIR/out/ChromImpute "$BINDIR/run_ChromImpute.sh COEFFV $INFOFILE $SAMPLETABLE"
done < ${CHROMSIZES}
export CHROM="" # Must clear chromosome value.

# TODO add rerun script for longer reruns for some.
# =======================================================
...
# ===========================================
# 9a. TODO: Learn model for tracks set up above
