...
qsub -cwd -t 1-13440 -l h_vmem=12G -l h_rt=24:00:00 -N alltracks_compare -j y -b y -V -r y -o $DBDIR/out/ChromImpute "$BINDIR/run_distance_alltracks_v_alltracks.sh"


# 2. Aggregate all of the distance calculations:
cd $CIDIR/alltracks_compare_distance
while read file; do
    echo $file
    id=${file%_all.txt}
    samp=${id%_obs_*}
...
# TODO: not exact - needs var calls.
...
# 2. Aggregate all of the distance calculations (NTOP=10):
