...
plot.avail <- function(mat, colramp = heat.colors(12), title='', with.rownames=FALSE,
                       add.cutoffs=TRUE, horiz=TRUE, legend='', cex.lab='auto', 
                       highlightct=c(), highlightct2=c()){
    if (horiz){ 
        mat = t(mat) 
        par(mar=c(6 + with.rownames * 7, 7, 3, 2)) 
    } else {
        par(mar=c(9, 4 + with.rownames * 7, 3, 2)) 
    }
    ...
    if (cex.lab == 'auto'){
        NS = nrow(mat)
        if (NS > 160){ cex.lab = 0.35 } else { cex.lab = 1.5 - 1.15 * NS / 160.0 }
    }
    if (horiz){
    ...
    if (with.rownames * horiz){
        ...
        ids2 = which(rownames(mat) %in% highlightct2)
        bothids = unique(c(ids,ids2))
        if (length(ids) > 0){
            if (length(rownames(mat)[ids]) > 0){
                text(x=xt[ids], y=yt, labels=rownames(mat)[ids], col='red', srt=90, adj=1, xpd=TRUE,cex=cex.lab) }
            if (length(rownames(mat)[ids2]) > 0){
                text(x=xt[ids2], y=yt, labels=rownames(mat)[ids2], col='royalblue', srt=90, adj=1, xpd=TRUE,cex=cex.lab) }
            if (length(rownames(mat)[-bothids]) > 0){
                text(x=xt[-bothids], y=yt, labels=rownames(mat)[-bothids], srt=90, adj=1, xpd=TRUE,cex=cex.lab) }
